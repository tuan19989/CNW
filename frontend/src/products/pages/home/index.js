...
function Home() {
  ...
  const [listProducts, setListProducts] = React.useState([]); //Create listCustomers State
  ...
  const RetrieveAllProducts = () => {
    ...
    CRUD.getAllPor().then((res) => {
      //console.log(res);
      setListProducts(res.data); //Set list customers after get all result from server
