...
function Home() {
  ...
  const [listOrderDetail, setListOrderDetail] = React.useState([]); //Create listCustomers State
  ...
  const RetrieveAllOrderDetail = () => {
    ...
    console.log("Retrieve all OrderDetail");
    CRUD.getAllOD().then((res) => {
      //console.log(res);
      setListOrderDetail(res.data); //Set list customers after get all result from server
  ...
  React.useEffect(() => {
    RetrieveAllOrderDetail(); //Retrieve data when component rendered
  }, [checkUpdate]); //Dependencies, checkUpdate thay đổi => chạy lại useEffect

  return (
    // Short hand React.Fragment
    <Container fluid={true}>
      <h2 className="text-center">OrderDetail</h2>
      <Row>
        <Col xs="8">
          <TableContent
        ...
        <Col xs="4">
          <h3 className="text-center">Create OrderDetail</h3>
