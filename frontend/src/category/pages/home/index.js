...
function Home() {
  ...
  const [listcategory, setListcategory] = React.useState([]); //Create listCustomers State
  ...
  const RetrieveAllcategory = () => {
    ...
    console.log("Retrieve all category");
    CRUD.getAllCate().then((res) => {
      //console.log(res);
      setListcategory(res.data); //Set list customers after get all result from server
  ...
  React.useEffect(() => {
    RetrieveAllcategory(); //Retrieve data when component rendered
